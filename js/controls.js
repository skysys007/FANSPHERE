...
window.addEventListener('crowdIntelligenceUpdated', (e) => {
  ...
  const avgSpeed =
    data.sections.reduce((sum, s) => sum + parseFloat(s.movementSpeed), 0) /
  ...
  const isSimRunning =
    (window.stadiumSim && window.stadiumSim.isRunning) ||
    (dataEngineSelect && dataEngineSelect.value !== 'math');
      ...
      if (ev.target.tagName === 'BUTTON' && ev.target.dataset.actionId) {
        ...
        if (actionId === 'deployStaff') {
          if (window.activeEmergencies) {
            const em = window.activeEmergencies.find(
              (e) => e.status === 'active' && e.location === locationsStr
            );
            if (em) {
              em.status = 'resolved';
          ...
          ev.target.className =
            'w-full py-2 mt-2 text-xs font-bold rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30';
          ...
          if (window.stadiumSim && window.stadiumSim.intelligenceEngine) {
            ...
            window.dispatchEvent(
              new CustomEvent('crowdIntelligenceUpdated', { detail: intelOutput })
            );
          }
          return;
        }
        ...
        if (window.stadiumSim && locationsStr) {
          ...
          locations.forEach((loc) => {
            window.stadiumSim.intelligenceEngine.executeAction(
              window.stadiumSim,
              actionId,
              loc.trim()
            );
  ...
  const hasActiveIncidents = displayAlerts.some(
    (a) => !a.lifecyclePhase || a.lifecyclePhase === 'taking_action'
  );
  ...
  if (isSimRunning) {
    const gateNames = {
      'gate-amex': 'AMEX Gate',
      'gate-hcl': 'HCLTech Gate',
      'gate-verizon': 'Verizon Gate',
      'gate-fansphere': 'FANSPHERE Gate',
    ...
    const formattedGates = (data.gates || []).map((g) => {
      ...
      return {
        ...
        congestionPercentage: (() => {
          ...
          if (window.stadiumSim) {
            const simGate =
              window.stadiumSim.entryGates.get(g.id) || window.stadiumSim.exitGates.get(g.id);
            if (simGate) {
              const prev =
                simGate._lastUI_Congestion !== undefined ? simGate._lastUI_Congestion : target;
    ...
    allZones.sort((a, b) => {
      const scoreA =
        a.riskLevel === 'Critical' ? 1000 : a.riskLevel === 'High' ? 500 : a.congestionPercentage;
      const scoreB =
        b.riskLevel === 'Critical' ? 1000 : b.riskLevel === 'High' ? 500 : b.congestionPercentage;
    ...
    zoneGrid.innerHTML = DOMPurify.sanitize(allZones)
      .map((s) => {
        ...
        if (s.crowdStatus === 'Congested') {
          riskColor = 'text-red-400';
          barColorCong = 'bg-red-500';
        }
        if (s.crowdStatus === 'Critical') {
          riskColor = 'text-red-500 font-black animate-pulse';
          barColorCong = 'bg-red-600';
        }
  ...
  if (data.gates) {
    data.gates.forEach((g) => {
      ...
      if (gate) {
        ...
        gate.status =
          gate.queueLength > 80 ? 'congested' : gate.queueLength > 2 ? 'open' : 'closed';
      }
    });
  }
  ...
  if (data.sections) {
    data.sections.forEach((s) => {
      ...
      if (sec) {
        ...
        sec.peopleEnteringPerMin =
          s.peopleEnteringPerMin !== undefined ? s.peopleEnteringPerMin : sec.peopleEnteringPerMin;
        sec.peopleLeavingPerMin =
          s.peopleLeavingPerMin !== undefined ? s.peopleLeavingPerMin : sec.peopleLeavingPerMin;
    ...
    data.zones.forEach((zone) => {
      ...
      if (!targetObj) {
        ...
        if (secMatch) {
          ...
          for (const sec of sim.sections.values()) if (sec.name === fallbackName) targetObj = sec;
        }
      }
