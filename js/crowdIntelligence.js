...
class CrowdIntelligenceEngine {
  ...
  _mapSimulationStateToInput(sim, timestamp) {
    ...
    sim.sections.forEach((section) => {
      ...
      sections.push({
        ...
        occupancyPercentage:
          section.capacity > 0 ? (section.occupiedSeats / section.capacity) * 100 : 0,
  ...
  _generateIntelligence(input) {
    ...
    input.sections.forEach((sec) => {
      ...
      const stateTracker = this.zoneStateHistory.get(sec.id) || {
        critical: 0,
        congested: 0,
        busy: 0,
        lastScore: rawTarget,
      ...
      if (congestionScore > 65) stateTracker.critical++;
      else stateTracker.critical = 0;
      if (congestionScore > 50) stateTracker.congested++;
      else stateTracker.congested = 0;
      if (congestionScore > 35) stateTracker.busy++;
      else stateTracker.busy = 0;
      ...
      const actionId = null;
      const aiExplanation = 'Normal crowd flow.';
      const actionImpact = null;
      ...
      if (status === 'Critical') {
        if (activeAction && activeAction.phase !== 'cooldown') {
          ...
          if (activeAction.phase === 'taking_action')
            msg = 'Deploying staff and redirecting spectators...';
          if (activeAction.phase === 'resolving')
            msg = 'Opening alternate routes. Crowd conditions improving...';
    ...
    input.gates.forEach((gate) => {
      ...
      if (activeAction && activeAction.phase !== 'cooldown') {
        ...
        const formattedWait = (() => {
          if (
            gate.estimatedWaitTime === null ||
            gate.estimatedWaitTime === undefined ||
            isNaN(gate.estimatedWaitTime)
          )
            return '—';
        ...
        const formattedQueue = isNaN(gate.queueLength)
          ? '—'
          : `${Math.round(gate.queueLength)} people`;
  ...
  _predict(history, futureMs) {
    ...
    const prediction = last.val + rate * futureMs;
