  ...
  setPhase(phase) {
    ...
    if (phase === 'halftime') {
      ...
      this.sections.forEach((sec) => {
        const targetOccupancy = sec.capacity * (0.9 + Math.random() * 0.05);
      ...
      this.sections.forEach((sec) => {
        const targetOccupancy = sec.capacity * (0.7 + Math.random() * 0.1);
        if (sec.occupiedSeats < targetOccupancy) {
          sec.occupiedSeats = targetOccupancy;
          this._updateSectionMetrics(sec);
        }
  ...
  processExternalFlow(minElapsed) {
    if (this.phase === 'pre_match') {
      ...
      this.entryGates.forEach((gate) => {
        ...
        if (gate.capacityModifier)
          maxCanProcess = Math.floor(maxCanProcess * gate.capacityModifier);
      ...
      this.entryGates.forEach((gate) => {
        ...
        gate.queueLength = Math.max(
          2,
          Math.floor(gate.queueLength * 0.8) + Math.floor(Math.random() * 8)
        );
        gate.density = 0;
        gate.status = 'normal';
      });
      ...
      this.exitGates.forEach((gate) => {
        ...
        gate.queueLength = Math.max(
          5,
          Math.floor(gate.queueLength * 0.85) + Math.floor(Math.random() * 12)
        );
  ...
  processInternalFlow(minElapsed) {
    if (this.phase === 'pre_match') {
      ...
      if (unfilledSections.length > 0 && newlyEntered > 0) {
        ...
        const hotspotPeople = hotspots.length > 0 ? newlyEntered * 0.45 : 0;
        const normalPeople = newlyEntered - hotspotPeople;
      ...
      this.sections.forEach((sec) => {
        if (sec.activeIncidentTicks > 0) {
          ...
          // 0.2% chance to start a new incident
